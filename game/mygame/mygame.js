...
window.onload = function() {
  ...
  canvas.style.width = W_WIDTH*scaleRate*0.8+'px';
  canvas.style.height = W_HEIGHT*scaleRate*0.8+'px';  // キャンバスを引き伸ばし
  ...
  for(var i = 0; i < C_HEIGHT; i++) {
    for(var j = 0; j < C_WIDTH; j++) {
      if(cell[i][j] != 0) {
        ...
        if(num >= DEL_NUM) {
          deletePuyoJoin(j, i, cell[i][j]);
          /*
          while(deletePuyoJoin(j, i, cell[i][j])) {
            while(dropBlock());
          }
          */
        }
...
function countPuyoJoin(x, y) {
  ...
  if (y+1<C_HEIGHT && cell[y+1][x]==c) n += countPuyoJoin(x, y+1);
  if (x-1>=0 && cell[y][x-1]==c) n += countPuyoJoin(x-1, y);
  if (x+1<C_WIDTH && cell[y][x+1]==c) n += countPuyoJoin(x+1, y);
...
function makePuyo() {
  for(var i = 0; i < C_WIDTH; i++) {
    if(cell[0][i] == 0) {
      cell[0][i] = Math.floor(Math.random() * COLOR_NUM);
    }
  }
}
...
function drawCell() {
  for(var i = 0; i < C_HEIGHT; i++) {
    for(var j = 0; j < C_WIDTH; j++) {
      switch(cell[i][j]) {
      case 0:
        context.fillStyle = 'rgb(255, 255, 255)';          // 色update();
        break;
      case 1:
      ...
      case 3:
        ...
        break;
      case 4:
        context.fillStyle = 'rgb(255, 255, 100)';          // 色update();
        break;
      }
